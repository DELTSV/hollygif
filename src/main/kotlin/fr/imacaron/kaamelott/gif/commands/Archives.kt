package fr.imacaron.kaamelott.gif.commands
...
import fr.imacaron.kaamelott.gif.repository.GifRepository
...
class Archives(
	...
	suspend operator fun invoke(interaction: GuildChatInputCommandInteraction) {
		...
		val gifs = gifRepository.getUserGifs(interaction.user.id, page)
		resp.respond {
			embed {
				title = "Archives"
				if(gifs.isEmpty()) {
