...
class DbSeriesRepository(
	...
	override fun getSeries(): Result<List<SeriesEntity>> = Result.success(db.series.toList())

	override fun getSeries(name: String): Result<SeriesEntity> = db.series.find { SeriesTable.name eq name }?.let {
		Result.success(it)
	} ?: Result.failure(NotFoundException("Series not found"))

	override fun addSeries(series: SeriesEntity): Result<SeriesEntity>{
